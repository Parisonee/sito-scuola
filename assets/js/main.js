...
(function ($) {
  ...
  jQuery(document).on("ready", function () {
    ...
    function submitContactForm(captcha) {
      ...
      function success(s) {
        if (s?.success === false) {
          // to give time to disappear the previous
          setTimeout(() => {
            error(s["error-codes"]);
          }, 1000);
          return;
        ...
        $("#formToastTitle").html("Grazie! 🚀");
        $("#formToastText").html(
          "Richiesta di contatto correttamente inviata!"
        );
        setTimeout(() => {
      ...
      function error(e) {
        ...
        $("#formToastTitle").html("Attenzione! 🚨");
        $("#formToastText").html("Qualcosa è andato storto, riprovare!");
        $("#formToast").toast("show");
      }
    ...
    form.addEventListener("submit", function (e) {
      ...
      grecaptcha.ready(function () {
        grecaptcha
          .execute("6LfDpScjAAAAAAsgdRdxpdJOBZTlHiXJVRxVkrt0", {
            action: "submit",
          })
    ...
    // Add smooth scrolling on all links inside the price cards
    $("#Pricing a").on("click", function (event) {
      // Make sure this.hash has a value before overriding default behavior
      if (this.hash !== "") {
        // Prevent default anchor click behavior
        event.preventDefault();
